...
@Component({
  selector: 'app-headbillingpayment',
  templateUrl: './headbillingpayment.component.html',
  styleUrls: ['./headbillingpayment.component.css']
})
export class HeadbillingpaymentComponent {
  ...
  initialiseGroupPaymentItemsForm() {
    this.groupPaymentItemsForm = this.formBuilder.group({
      amountAssessed:new FormControl({ value: this.singlePartialPayData?.amount_due,
         disabled: true }), 
...
goToLink(url: string) {
  ...
  public setItemsPerPageSettlement($event: any) {
    ...
    this.router.navigate(["/headbillingpayment"]);
