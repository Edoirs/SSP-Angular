...
@Component({
  selector: 'app-headbillingpaymenthistry',
  templateUrl: './headbillingpaymenthistry.component.html',
  styleUrls: ['./headbillingpaymenthistry.component.css']
})
export class HeadbillingpaymenthistryComponent {
  userId: any
  ...
  getPaymentHistory(perpage: any, pageno: any) {
    ...
    this.apiUrl = `${environment.AUTHAPIURL}admins/reports/payment-history?per_page=${perpage}&page=${pageno}`;
  ...
  onSubmitSearch() {
    ...
    this.apiUrl = `${environment.AUTHAPIURL}admins/reports/payment-history?per_page=${perpage}&page=${pageno}&consumer_sub_category_id=${consumerSubcategory}&tariff_type_id=${tariffId}&date_range=${this.startedDate}`;
    const reqHeader = new HttpHeaders({
      "Content-Type": "application/json",
      "Authorization": "Bearer " + localStorage.getItem("niswasec_access_token"),
    });
  ...
  public setItemsPerPage($event: any) {
    ...
    this.router.navigate(["/headbillingpaymenthistory"]);
