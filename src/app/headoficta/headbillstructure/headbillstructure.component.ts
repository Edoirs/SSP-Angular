...
@Component({
  selector: 'app-headbillstructure',
  templateUrl: './headbillstructure.component.html',
  styleUrls: ['./headbillstructure.component.css']
})
export class HeadbillstructureComponent {
  ...
  initialisecreatesubCategoryForms() {
    this.createSubcategoryForm = this.formBuilder.group({
      ...
      category: 
      new FormControl({ value: this.selectedSubcategory, disabled: true},[
        Validators.required
      ]),
    });
  }
  ...
  category(activeTab: string, $event: MouseEvent): void {
    ...
    this.router.navigate(["/headbillstructure"]);
      this.config = {
        currentPage: 1,
        itemsPerPage: 10,
      };
  ...
  getCompByID($event:any){
    this.compId = $event?.target.value;
    this.getTariffByComp(this.compId)
  }
  getTariffByComp(comp:any){
    if(comp == 1){
      this.tariffData = this.tariffTypes.filter((i: any) => i.id != comp);
    }
    else if(comp == 2){
      this.tariffData = this.tariffTypes.filter((i: any) => i.id != comp);
    } 
  ...
  getbillCycle($event:any){
    this.billId = $event?.target.value;
    this.getBillcycle(this.billId)
  }

  getBillcycle(billId:any){
    if(billId == 2){
      this.billingcyclesdata = this.billingcycles.filter((i: any) => i.id != billId);
    }
   else{
    return this.billingcyclesdata = this.billingcycles;
   }
  }
  ...
  onEditsubCategory(modal: any, selectedData: any) {
    ...
    this.selectedCategoryId = selectedData.category_id;
    this.editSubcategoryForm = this.formBuilder.group({
      name: [selectedData.name, [
        Validators.required
      ]],
      category:  new FormControl({
        value: selectedData.category_id, disabled: true
      }, [Validators.required]),
    });
  }
