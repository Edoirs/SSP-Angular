...
@Component({
  selector: 'app-selflogin',
  templateUrl: './selflogin.component.html',
  styleUrls: ['./selflogin.component.css']
})
export class SelfloginComponent implements OnInit {
      ...
      checkEmail(formAllData: any) {
        ...
        this.apiUrl = environment.AUTHAPIURL + "self-portal/auth/send-otp";
      ...
      onOtpSubmit(formAllData: any) {
        ...
        this.ngxService.start();
        this.apiUrl = environment.AUTHAPIURL + "self-portal/auth/login";
        this.http.post<any>(this.apiUrl, otpObj).subscribe((data) => {
          ...
          if (this.defaultPwd == false) {
            ...
            localStorage.setItem("niswasec_username", data.response?.email);
            this.router.navigate(["/selfresetpassword"]);
          }
          ...
          if (data.status == true) {
            ...
            this.router.navigate(["/selfdashboard"])
          }
